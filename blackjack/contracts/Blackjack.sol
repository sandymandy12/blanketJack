...
contract Game is ERC721 {
    ...
    modifier isActive(uint _gameId) {
        Game memory game = idToGameInfo[_gameId];
        require(game, "Must be a valid game");
        _;
    }
    ...
    function createGame(uint256 _gameId, uint _size) public { // Passing the name and tokenId as a parameters
    ...
    function startGame(uint _gameId, uint _size) public {
        Game memory newGame = idtoGameInfo[_gameId];
        require(newGame.size == newGame.players.length, "Not enough players);
        _mint(msg.sender, _gameId); // _mint assign the the star with _tokenId to the sender address (ownership)
    }

    function joinGame(uint _gameId) public isActive(_gameId) {
        Game memory game = idToGameInfo[_gameId];
        require(!game.players[msg.sender], "Player already joined game");
        require(game.players.length < size, 'Maximum capacity');
        game.players.push(msg.sender);
    }
    
    function leaveGame(uint _gameId) public isActive(_gameId) {
        Game memory game = idToGameInfo[_gameId];
        game.players.pop(msg.sender);
